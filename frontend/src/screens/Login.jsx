...
import {Failed, Warning, Success} from '../components/Category/Alerts/Alerts'
...
function Login() {
  ...
  const loginHandler = async (e) => {
    ...
    //Checking if User Inout is Empty

    if(!form.email || !form.password){
      setShowErrors(true)
      setError('Please ensure you email and password are provided')
      setTimeout(() => {
        setShowErrors(false);
      }, 3000);
      return
    ...
    // Making A API Call to the Backend For Login
    ...
    try {
      setLoading(true)
      const response = await axios.post("/api/v1/users/login", form, config)
      console.log(response)

      if(response.data.status === 'success'){
        SweetAlert('success', 'Login Successful', 'You are now logged in')
        setTimeout(() => {
          navigate('/')
        }, 2000)
      }
  ...
  <div className="container px-6 py-12 h-full">
    ...
    <div className="flex justify-center items-center flex-wrap h-full g-6 text-gray-800">
      ...
      <div className="md:w-8/12 lg:w-5/12 lg:ml-20">
        {showError && (<Failed message={error} />)}
        ...
        <form onSubmit={loginHandler}>
          ...
          <div className="grid sm:flex justify-between items-center mb-6">
            ...
            <div>
            <a href="#!" className="text-blue-600 hover:text-blue-700 focus:text-blue-700 active:text-blue-800 duration-200 transition ease-in-out hover:text-yellow" >Forgot password?</a>
            </div>

            <div>
            <Link to="/register" className="text-blue-600 hover:text-blue-700 focus:text-blue-700 active:text-blue-800 duration-200 transition ease-in-out hover:text-yellow">New on Jobcy? Register Here</Link>
            </div>
          ...
          <button type="submit" className="inline-block px-7 py-3 bg-yellow text-black font-bold text-xl leading-snug uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out w-full" data-mdb-ripple="true" data-mdb-ripple-color="light">
            Sign in
          </button>
